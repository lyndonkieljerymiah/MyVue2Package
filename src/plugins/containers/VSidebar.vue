...
<script>
const jq = window.$ || require('jquery');
...
export default {
    ...
    mounted() {
        ...
        jq('.nb-dropdown-menu').hide();
        
        jq(dropdowns).removeClass('active');
        
        jq(dropdowns).on('click', function(e) {
            jq('.nb-dropdown-menu').hide();
            jq(dropdowns).parent().removeClass('active');
